...
def segment_ids(transcript: Transcript) -> list[str]:
...
def evidence_segment_ids_for_span(transcript: Transcript, start: float | None, end: float | None) -> list[str]:
    ...
    seg_ids = segment_ids(transcript)
    overlaps: list[str] = []
    for i, seg in enumerate(transcript.segments):
        if seg.end >= start and seg.start <= end:
            overlaps.append(seg_ids[i])
    return overlaps


def evidence_segment_ids_for_text(transcript: Transcript, text: str) -> list[str]:
    ...
    for i, seg in enumerate(transcript.segments):
        if needle in seg.text.lower():
            hits.append(seg_ids[i])
    return hits


def evidence_for_entity(transcript: Transcript, ent: Entity) -> list[str]:
    by_span = evidence_segment_ids_for_span(transcript, ent.start, ent.end)
    if by_span:
        return by_span
    return evidence_segment_ids_for_text(transcript, ent.text)
...
def events_by_type(events: Sequence[_Event], event_type: str) -> list[_Event]:
...
def build_claim(
    ...
    def add_provenance(segment_ids: Iterable[str], note: str) -> None:
        for seg_id in unique_strs(segment_ids):
            provenance_links.append(ProvenanceLink(segment_id=seg_id, note=note))

    # location_hint
    loc = first_entity(entities, {"location", "address", "cross_street"})
    if loc is not None:
        i, ent = loc
        seg_ids = evidence_for_entity(transcript, ent)
    ...
    call_events = events_by_type(events, "CALL_RECEIVED")
    dispatched_events = events_by_type(events, "DISPATCHED")
    for ev in [*call_events, *dispatched_events]:
        dispatch_evidence.extend(ev.get("evidence_segment_ids") or [])

    inc = first_entity(entities, {"incident_type", "call_type", "dispatch_reason"})
    if inc is not None:
        i, ent = inc
        seg_ids = evidence_for_entity(transcript, ent)
    ...
    pr = first_entity(entities, {"priority", "acuity"})
    if pr is not None:
        i, ent = pr
        seg_ids = evidence_for_entity(transcript, ent)
        dispatch_evidence.extend(seg_ids)
    ...
    unit_entities = all_entities(entities, {"unit", "unit_id"})
    ...
    age = first_entity(entities, {"age", "patient_age"})
    sex = first_entity(entities, {"sex", "gender", "patient_sex"})
    if age is not None:
        i, ent = age
        seg_ids = evidence_for_entity(transcript, ent)
    ...
    cc = first_entity(entities, {"chief_complaint", "complaint", "primary_impression"})
    if cc is None:
        cc = first_entity(entities, {"symptom"})
    if cc is not None:
        i, ent = cc
        seg_ids = evidence_for_entity(transcript, ent)
    ...
    for i, ent in all_entities(entities, {"vital", "vitals"}):
        seg_ids = evidence_for_entity(transcript, ent)
    ...
    for i, ent in all_entities(entities, {"medication", "procedure", "intervention", "treatment"}):
        seg_ids = evidence_for_entity(transcript, ent)
    ...
    if events_by_type(events, "TRANSPORT_BEGIN") or events_by_type(events, "ARRIVE_ED"):
        status = "transported"
        for ev in [*events_by_type(events, "TRANSPORT_BEGIN"), *events_by_type(events, "ARRIVE_ED")]:
            disp_evidence.extend(ev.get("evidence_segment_ids") or [])

    disp_ent = first_entity(entities, {"disposition"})
    ...
    dest_ent = first_entity(entities, {"destination", "destination_hint", "hospital"})
    ...
    disposition["evidence_segment_ids"] = unique_strs(disp_evidence)
    ...
    used_segments = unique_strs([p.segment_id for p in provenance_links])
